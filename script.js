...
// Auto-format date inputs as MM/DD/YYYY while typing
...
// Attach event listeners for auto-formatting on date inputs
document.getElementById("startDate").addEventListener("input", autoFormatDateInput);
document.getElementById("endDate").addEventListener("input", autoFormatDateInput);

// Parses MM/DD/YYYY string into Date object; returns null if invalid
function parseDDMMYYYY(input) {
  const raw = input.replace(/\D/g, ""); // remove slashes
...
function calculateLoan() {
  ...
  if (!startDate) {
    alert("ప్రారంభ తేదీ తప్పు ఉంది. దయచేసి MM/DD/YYYY ఫార్మాట్‌లో సరైన తేదీని నమోదు చేయండి.");
    return;
  }
  if (!endDate) {
    alert("ముగింపు తేదీ తప్పు ఉంది. దయచేసి MM/DD/YYYY ఫార్మాట్‌లో సరైన తేదీని నమోదు చేయండి.");
    return;
  }
